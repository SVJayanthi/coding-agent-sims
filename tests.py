...
# Import the non-Ray version for testing
import docker
import time
...
class LocalSimulationEnvironment:
    ...
    async def create_workspace(self, agent_id: str, task_spec: TaskSpec):
    ...
    async def execute_action(self, workspace_id: str, action):
        # Simplified version for testing - copy from sim_env.py
        if workspace_id not in self.workspaces:
            from data_structures import ActionResult
            return ActionResult(
                success=False,
                output="",
        ...
        # Import ActionType locally
        from data_structures import ActionType, ActionResult
    ...
    async def evaluate_progress(self, workspace_id: str):
...
class TestSimulationEnvironment:
    ...
    async def test_basic_file_operations(self):
        ...
        config = SimConfig()
        sim_env = LocalSimulationEnvironment(config)
